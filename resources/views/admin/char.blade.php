...
<div class="row">
    ...
    <hr>
    <button style="col-md-12" type="button" class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#orderDetails-">
        Add new category
    </button>
    <hr class="mt-3 mb-3">
</div>
            ...
            <div class="modal-body">
                <div class="row">
                    <form action="{{ route('addChar', $category->id) }}" method="POST">
                        ...
                        <div class="form-group mt-3">
                            <input style="text-align: center" type="text" name="numberInFilter" id="numberInFilter" placeholder="Number in filter..." class="form-control">
                        </div>
                        <hr>
...
<div class="row">
    ...
    @foreach ($charList as $item)
        ...
        <button type="button" class="btn btn-outline-success col-md-1" data-bs-toggle="modal" data-bs-target="#valuesModal-{{ $item->id }}">Values</button>
            <div class="modal fade" id="valuesModal-{{ $item->id }}" tabindex="-1" aria-labelledby="valuesModalLabel-{{ $item->tittle }}" aria-hidden="true">
            ...
            <button type="button" class="btn btn-outline-success col-md-2" data-bs-toggle="modal" data-bs-target="#addValueModal-{{ $item->id }}">Add value</button>
            <div class="modal fade" id="addValueModal-{{ $item->id }}" tabindex="-1" aria-labelledby="addValueModalLabel-{{ $item->tittle }}" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header text-dark">
                        ...
                        <div class="modal-body text-dark">
                        <div>
                            <form action="{{ route('addCharValue', ['id'=>$category->id, 'charId'=>$item->id]) }}" method="POST">
                                @csrf
                                <div class="form-group mt-3">
                                    <input type="text" name="value" id="value" placeholder="Value..." class="form-control text-center">
                                </div>
                                <div class="form-group mt-3">
                                    <input type="text" name="numberInFilter" id="numberInFilter" placeholder="Number in filter..." class="form-control text-center">
                                </div>
                                <hr>
